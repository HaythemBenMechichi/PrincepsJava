...
public class FrontProduitController implements Initializable {
    ...
    String uploads = "C:\\Users\\Dhia werteteni\\Downloads\\wetransfer_princepsjava-rar_2022-05-11_2216\\PrincepsJava\\PrincepsJava\\princeps\\src\\edu\\workshopjdbc3a48";
    @FXML
    private Label DESC;
    @FXML
    ...
    private void AddPanier(ActionEvent event) {
        if (Produit.size() > 0) {
            Produit p = Produit.get(0);
            exists = false;
            int id = p.getId();
            Carte.getPanier().forEach(bol -> {
                if (bol.equals(p.getId())) {
                    exists = true;
                    return;
                }
            });
            if (exists == false) {
                ...
                a.setContentText("Produit " + p.getLibelle() + " est ajouté au panier!");
                a.showAndWait();
    ...
    @FXML
   private void panier(MouseEvent event) throws SQLException, ClassNotFoundException, IOException {
